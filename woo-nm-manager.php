...
register_activation_hook(__FILE__, function(){ if (!wp_next_scheduled('wnm_reconcile_stock')) wp_schedule_event(time()+300, 'hourly', 'wnm_reconcile_stock'); });
register_deactivation_hook(__FILE__, function(){ wp_clear_scheduled_hook('wnm_reconcile_stock'); });

add_action('plugins_loaded', function(){
    if (!class_exists('WooCommerce')) {
        add_action('admin_notices', function(){ echo '<div class="notice notice-error"><p>Woo NM Manager requires WooCommerce.</p></div>'; });
        return;
    }
